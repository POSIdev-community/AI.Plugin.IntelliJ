...
You can monitor the scan progress in the bottom panel or in the **Log** tab in the **PT Application Inspector** panel. The first scan usually takes longer due to the initial load on the database of vulnerable components.
...
In the `dev_mode_config.json` configuration file, you can configure user log tab display. By default, the file contains the path to output the analyzer log. To add extra tabs, in the **panelName** field, enter a tab name, and in the **pathsToLogs** field, enter at least one path to the log file that will be displayed on the tab.
